...
- **ブロックエディタ**: 段落・見出し・リスト・引用・画像・画像ギャラリーの6種類の決まったブロックを
  ...
  ブロックの追加は種類が増えても見やすいモーダル選択式、並べ替えはドラッグハンドルで行います。
...
- **固定ページ(Page)**: 会社概要のような更新頻度の低いページ用の機能。管理画面(`/pages`)から
  Postと同じブロックエディタで作成/修正でき、カテゴリー・作成者・要約の概念がない分シンプルです。
...
3. **記事を作成** — 「新規投稿」でタイトルを入力し、本文欄の「+ ブロックを追加」ボタンから
   開くモーダルで段落・見出し・画像・画像ギャラリーなどのブロックを選んで積み重ねます。
   「カスタムブロック」画面で作った独自ブロックがあれば、同じモーダルにその名前で並びます。
   各ブロックはドラッグハンドル(⠿)で並べ替えできます。画像系のブロックは
...
タグ・カスタム投稿タイプをまとめて取り込みます(タグはこのCMSにタグの概念がないため
カテゴリーへ合流します)。
...
- 本文が空(または変換の結果ブロックが1つも残らない)場合でもスキップせず、空のブロック
  配列として取り込みます(タイトルだけの項目や、このCMSが解釈できない本文を持つ項目も
  取りこぼしません)。
- WordPressの「限定公開(private)」を含め、`publish`/`future`以外のステータスはすべて
  下書き(DRAFT)として取り込まれます。
- **ACFなどのカスタムフィールド(記事・固定ページ)**: Post/Pageにはカスタムフィールドという
  概念がないため、`wp:postmeta`から検出したフィールドをこのCMSのカスタムブロック(BlockType)
  として自動生成し、本文Block配列の末尾にカスタムブロックとして追加します。記事用・固定
  ページ用でそれぞれ別のブロックタイプ(`wp-post-fields`/`wp-page-fields`)になり、
  カスタムフィールドを持つ項目が1件もなければブロックタイプ自体を作りません。
- **post/page以外の投稿タイプ**(WordPressのカスタム投稿タイプ)は、`wp:post_type`の値ごとに
  このCMSのカスタム投稿タイプ(PostType)として自動的に定義され(同じslugの定義が既にあれば
  再利用)、各項目がエントリー(PostTypeEntry)として取り込まれます。フィールド構成は
  本文・抜粋(あれば)・`wp:postmeta`から検出したキーで構成されます。`_`始まりの内部用
  メタキー(編集ロック、ACF等プラグインのフィールド定義参照キーなど)やPHPの
  `serialize()`形式の複合値(配列・チェックボックスグループ等)は取り込み対象から除外し、
  単純な文字列値だけを取り込みます。
- 添付ファイル(メディア)・メニュー・リビジョン、およびブロックテーマ内部用の投稿タイプ
  (`wp_global_styles`等)、代表的なプラグインが内部管理用に使う投稿タイプ(ACFの
  `acf-field-group`等、MW WP Formの`mw-wp-form`、LazyBlocksの`lazyblocks`等)、
  「ゴミ箱」「自動下書き」の項目は取り込み対象外です。
- WordPressの「予約投稿」は、日時が未来であれば予約公開(SCHEDULED)として取り込まれます。
- 項目ごとに成功/失敗を分離して処理するため、一部でエラーが出ても全体は中断せず、
  作成件数(記事・固定ページ・カテゴリー・カスタム投稿タイプ定義・エントリー・
  カスタムブロック定義)・スキップ件数・失敗件数がまとめて表示されます。
...
- 定義(`PostType`)ごとに、サイドバーへ専用の「エントリー管理」画面(`/entries/:slug`)が
  ...
  (DRAFT/SCHEDULED/PUBLISHED)・フィールド値・カテゴリーを持ち、Post/Pageと同じ予約公開・
  カテゴリー選択の仕組みが使えます(カテゴリーは投稿タイプをまたいで共有されます)。
