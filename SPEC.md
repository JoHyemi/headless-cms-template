...
- `post`/`page`以外の`wp:post_type`は、値ごとに1つのPostType定義として自動的に
  ...
  項目にあれば)と`wp:postmeta`から検出したキーを追加したものになる。
...
- `attachment`(添付ファイル)・`nav_menu_item`(メニュー)・`revision`(リビジョン)、および
  `wp_global_styles`/`wp_template`などのブロックテーマ内部用投稿タイプはインポート
  対象外としてスキップする(ユーザーが作成したコンテンツではないため)。
