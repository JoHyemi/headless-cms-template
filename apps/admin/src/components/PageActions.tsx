...
// 管理者の固定ページ一覧で使う「公開/下書き切り替え」及び「削除」ボタン(PostActionsと同じ形)。
export function PageActions({ id, status }: Props) {
  ...
  async function remove() {
    if (!confirm("この固定ページを削除しますか?元に戻せません。")) return;
    setPending(true);
    try {
  ...
  return (
    <div className="actions-row">
      ...
      <Link href={`/pages/${id}/edit`} className="btn">
        編集する
      </Link>
